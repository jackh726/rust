...
// The same obligation reached through a projection rather than an opaque: the bounds are the item
// bounds of an associated type, and the relation between them is declared on the trait. A fix that
// only looks at opaques misses this case.
mod projection {
    use super::require_static;
