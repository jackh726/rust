...
fn get_trait_predicates_for_trait_id<'tcx>(
    ...
    for (pred, _) in generics.predicates {
        if_chain! {
            let bound_pred = pred.bound_atom(cx.tcx);
            if let PredicateAtom::Trait(poly_trait_pred, _) = bound_pred.skip_binder();
            let trait_pred = cx.tcx.erase_late_bound_regions(&ty::Binder::rebind(poly_trait_pred, bound_pred.bound_vars()));
...
fn get_projection_pred<'tcx>(
    ...
    generics.predicates.iter().find_map(|(proj_pred, _)| {
        let bound_proj_pred = proj_pred.bound_atom(cx.tcx);
        if let ty::PredicateAtom::Projection(proj_pred) = bound_proj_pred.skip_binder() {
            let projection_pred = cx.tcx.erase_late_bound_regions(&ty::Binder::rebind(proj_pred, bound_proj_pred.bound_vars()));
