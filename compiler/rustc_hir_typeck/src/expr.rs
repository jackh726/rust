    ...
    fn check_expr_array(
        ...
        let element_ty = if !args.is_empty() {
            ...
            for e in args {
                ...
                let _expected = if coerce_to.is_fn_ptr() {
                    coerce_to
                } else {
                    coerce.merged_ty()
                };
                let e_ty = self.check_expr_with_hint(e, coerce.merged_ty());
                let cause = self.misc(e.span);
                coerce.coerce(self, &cause, e, e_ty);

                if first {
                    // HACK: This is needeed because of `check_expr_with_hint`
                    // is *stronger* than typical coercions, so we need to
                    // make sure that any info gained during LUB coercion is
                    // propogated back to the expected ty immediately.
                    //coerce.complete(self, &cause, true);
                    first = false;
                }
            }
