...
impl<D, Ir, I> assembly::GoalKind<D> for TraitPredicate<I>
where
    D: SolverDelegate<Interner = I, Ir = Ir>,
    ...
    <I as Interner>::AdtDef: IrAdtDef<I, D::Ir>,
{
    ...
    fn consider_impl_candidate(
        ...
        ecx.probe_trait_candidate(CandidateSource::Impl(impl_def_id)).enter(|ecx| {
            ...
            let goal_clause: I::Clause = goal.predicate.clone().upcast(cx.interner());
            for clause in elaborate::elaborate(cx.interner(), [goal_clause]) {
                if matches!(
                    ...
                    ecx.add_goal(
                        GoalSource::Misc,
                        goal.clone().with(cx.interner(), clause.clone()),
                    );
                }
            }
    ...
    fn consider_auto_trait_candidate(
        ...
        if let ty::Alias(ty::Opaque, opaque_ty) = goal.predicate.clone().self_ty().kind() {
    ...
    fn consider_trait_alias_candidate(
        ...
        ecx.probe_builtin_trait_candidate(BuiltinImplSource::Misc).enter(|ecx| {
            let nested_obligations = cx
                .interner()
                .predicates_of(goal.predicate.clone().def_id())
                .iter_instantiated(cx.interner(), goal.predicate.trait_ref.args.clone())
                .map(|p| goal.clone().with(cx.interner(), p));
    ...
    fn consider_builtin_fn_ptr_trait_candidate(
        ...
        let self_ty = goal.predicate.clone().self_ty();
        match goal.predicate.polarity {
            ...
            ty::PredicatePolarity::Negative => {
                ...
                if !self_ty.clone().is_fn_ptr() && self_ty.is_known_rigid() {
                    ecx.probe_builtin_trait_candidate(BuiltinImplSource::Misc).enter(|ecx| {
                        ecx.evaluate_added_goals_and_make_canonical_response(Certainty::Yes)
                    })
                } else {
    ...
    fn consider_builtin_fn_trait_candidates(
        ...
        let output_is_sized_pred = tupled_inputs_and_output.clone().map_bound(|(_, output)| {
        ...
        let pred = tupled_inputs_and_output
            .map_bound(|(inputs, _)| {
                ty::TraitRef::new(cx.interner(), goal.predicate.clone().def_id(), [
                    goal.predicate.clone().self_ty(),
                    inputs,
                ])
            })
        ...
        Self::probe_and_consider_implied_clause(
            ...
            goal.clone(),
            pred,
            [(GoalSource::ImplWhereBound, goal.clone().with(cx.interner(), output_is_sized_pred))],
        )
    }
    ...
    fn consider_builtin_async_fn_trait_candidates(
        ...
        let (tupled_inputs_and_output_and_coroutine, nested_preds) =
            structural_traits::extract_tupled_inputs_and_output_from_async_callable(
                ...
                Region::new_static(cx.interner()),
            )?;

        // A built-in `AsyncFn` impl only holds if the output is sized.
        // (FIXME: technically we only need to check this if the type is a fn ptr...)
        let output_is_sized_pred = tupled_inputs_and_output_and_coroutine.clone().map_bound(
        ...
        Self::probe_and_consider_implied_clause(
            ...
            [goal.clone().with(cx.interner(), output_is_sized_pred)]
                .into_iter()
                .chain(nested_preds.into_iter().map(|pred| goal.clone().with(cx.interner(), pred)))
    ...
    fn consider_builtin_future_candidate(
        ...
        let cx = ecx.cx();
        if !cx.interner().coroutine_is_async(def_id) {
            return Err(NoSolution);
        }
    ...
    fn consider_builtin_iterator_candidate(
        ...
        let cx = ecx.cx();
        if !cx.interner().coroutine_is_gen(def_id) {
            return Err(NoSolution);
        }
    ...
    fn consider_builtin_async_iterator_candidate(
        ...
        // Coroutines are not iterators unless they come from `gen` desugaring
        let cx = ecx.cx();
        if !cx.interner().coroutine_is_async_gen(def_id) {
            return Err(NoSolution);
        }
    ...
    fn consider_builtin_coroutine_candidate(
        ...
        let self_ty = goal.predicate.clone().self_ty();
        let ty::Coroutine(def_id, args) = self_ty.clone().kind() else {
            return Err(NoSolution);
        };

        // `async`-desugared coroutines do not implement the coroutine trait
        let cx = ecx.cx();
        if !cx.interner().is_general_coroutine(def_id) {
            return Err(NoSolution);
        }

        let coroutine = args.as_coroutine();
        let goal_predicate_def_id = goal.predicate.clone().def_id();
        Self::probe_and_consider_implied_clause(
            ecx,
            CandidateSource::BuiltinImpl(BuiltinImplSource::Misc),
            goal,
            ty::TraitRef::new(cx.interner(), goal_predicate_def_id, [
                self_ty,
                coroutine.resume_ty(),
            ])
            .upcast(cx.interner()),
    ...
    fn consider_builtin_transmute_candidate(
        ...
        ecx.probe_builtin_trait_candidate(BuiltinImplSource::Misc).enter(|ecx| {
            let assume = ecx.structurally_normalize_const(
                goal.param_env.clone(),
                goal.predicate.trait_ref.args.clone().const_at(2),
            )?;

            let certainty = ecx.is_transmutable(
                &goal.param_env,
                goal.predicate.trait_ref.args.clone().type_at(0),
    ...
    fn consider_structural_builtin_unsize_candidates(
        ...
        ecx.probe(|_| ProbeKind::UnsizeAssembly).enter(|ecx| {
            let a_ty = goal.predicate.clone().self_ty();
            // We need to normalize the b_ty since it's matched structurally
            // in the other functions below.
            let Ok(b_ty) = ecx.structurally_normalize_ty(
                goal.param_env.clone(),
                goal.predicate.trait_ref.args.clone().type_at(1),
            ) else {
                return vec![];
            };

            let goal = goal.clone().with(ecx.cx().interner(), (a_ty.clone(), b_ty.clone()));
            match (a_ty.clone().kind(), b_ty.clone().kind()) {
                ...
                (ty::Tuple(a_tys), ty::Tuple(b_tys))
                    if a_tys.clone().len() == b_tys.clone().len() && !a_tys.clone().is_empty() =>
                {
    ...
    <I as Interner>::AdtDef: IrAdtDef<I, Ir>,
{
    ...
    fn consider_builtin_dyn_upcast_candidates(
        ...
        let b_principal_def_id = b_data.clone().principal_def_id();
        if a_data.clone().principal_def_id() == b_principal_def_id || b_principal_def_id.is_none() {
            ...
            for new_a_principal in elaborate::supertraits(
                self.cx().interner(),
                a_principal.with_self_ty(cx.interner(), a_ty),
            )
            .skip(1)
            {
                responses.extend(self.consider_builtin_upcast_to_principal(
                    ...
                    Some(new_a_principal.map_bound(|trait_ref| {
                        ty::ExistentialTraitRef::erase_self_ty(cx.interner(), trait_ref)
                    })),
                ));
            }
        }
    ...
    fn consider_builtin_unsize_to_dyn_candidate(
        ...
        let cx = self.cx();
        let Goal { predicate: (a_ty, _), .. } = goal.clone();

        // Can only unsize to an dyn-compatible trait.
        if b_data
            .clone()
            .principal_def_id()
            .is_some_and(|def_id| !cx.interner().trait_is_dyn_compatible(def_id))
        {
            return Err(NoSolution);
        }
        ...
        self.probe_builtin_trait_candidate(BuiltinImplSource::Misc).enter(|ecx| {
            ...
            ecx.add_goals(
                ...
                b_data.iter().map(|pred| {
                    goal.clone().with(cx.interner(), pred.with_self_ty(cx.interner(), a_ty.clone()))
                }),
            );
            ...
            ecx.add_goal(
                GoalSource::Misc,
                goal.clone().with(cx.interner(), ty::OutlivesPredicate(a_ty, b_region)),
            );
            ecx.evaluate_added_goals_and_make_canonical_response(Certainty::Yes)
        })
    }
    ...
    fn consider_builtin_upcast_to_principal(
        ...
        let a_auto_traits: IndexSet<I::DefId> = a_data
            ...
            .auto_traits()
            .into_iter()
            .chain(a_data.clone().principal_def_id().into_iter().flat_map(|principal_def_id| {
                elaborate::supertrait_def_ids(self.cx().interner(), principal_def_id)
                    .filter(|def_id| self.cx().interner().trait_is_auto(*def_id))
            }))
            .collect();
            ...
            for bound in b_data.iter() {
                match bound.clone().skip_binder() {
                    ...
                    ty::ExistentialPredicate::Trait(target_principal) => {
                        ...
                        let target_principal = bound.clone().rebind(target_principal);
                    ...
                    ty::ExistentialPredicate::Projection(target_projection) => {
                        ...
                        let mut matching_projections =
                            a_data.clone().projection_bounds().into_iter().filter(
                                |source_projection| {
    ...
    fn consider_builtin_struct_unsize(
        ...
        let cx = self.cx();
        let Goal { predicate: (_a_ty, b_ty), .. } = goal.clone();

        let unsizing_params = cx.interner().unsizing_params_for_adt(def.def_id());
        ...
        let tail_field_ty = def.clone().struct_tail_ty(self.cx()).unwrap();

        let a_tail_ty = tail_field_ty.clone().instantiate(cx.interner(), a_args.clone());
        let b_tail_ty = tail_field_ty.instantiate(cx.interner(), b_args.clone());
        ...
        let new_a_args =
            cx.interner().mk_args_from_iter(a_args.iter().enumerate().map(|(i, a)| {
                if unsizing_params.contains(i as u32) { b_args.clone().get(i).unwrap() } else { a }
            }));
        let unsized_a_ty = Ty::new_adt(cx.interner(), def, new_a_args);
    ...
    fn consider_builtin_tuple_unsize(
        ...
        let unsized_a_ty = Ty::new_tup_from_iter(
            cx.interner(),
            a_rest_tys.iter().cloned().chain([b_last_ty.clone()]),
        );
        self.eq(goal.param_env.clone(), unsized_a_ty, b_ty)?;
    ...
    fn disqualify_auto_trait_candidate_due_to_possible_impl(
        ...
        match self_ty.clone().kind() {
            ...
            ty::Coroutine(def_id, _)
                if self
                    .cx()
                    .is_lang_item(goal.predicate.clone().def_id(), TraitSolverLangItem::Unpin) =>
            {
                match self.cx().interner().coroutine_movability(def_id) {
    ...
    fn probe_and_evaluate_goal_for_constituent_tys(
        ...
        self.probe_trait_candidate(source).enter(|ecx| {
            let goals = constituent_tys(ecx, goal.predicate.clone().self_ty())?
                .into_iter()
                .map(|ty| {
