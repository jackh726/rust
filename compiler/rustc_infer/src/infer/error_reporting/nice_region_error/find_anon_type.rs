...
impl<'a, 'tcx> NiceRegionError<'a, 'tcx> {
    pub(super) fn scope_of_region(&self, region: Region<'tcx>) -> Option<&hir::FnDecl<'tcx>> {
        self.tcx().is_suitable_region(region).and_then(|anon_reg| {
            let hir_id = self.tcx().hir().local_def_id_to_hir_id(anon_reg.def_id);
            Some(match self.tcx().hir().get(hir_id) {
    ...
    pub(super) fn find_anon_type(
        ...
        self.scope_of_region(region).and_then(|fndecl| {
            fndecl
                .inputs
                .iter()
    ...
    fn find_component_for_bound_region(
        ...
        let mut nested_visitor =
            FindNestedTypeVisitor { tcx: self.tcx(), bound_region: *br, found_type: None };
        ...
        match arg.kind {
            ...
            hir::TyKind::Rptr(ref lifetime, _) => {
                ...
                match (self.tcx.named_region(hir_id), self.bound_region) {
                    ...
                    (Some(rl::Region::LateBoundAnon(_, anon_index)), ty::BrAnon(br_index)) => {
                        debug!(
                            "LateBoundAnon anon_index = {:?} br_index={:?}",
                            anon_index, br_index
                        );
                        if anon_index == br_index {
                    ...
                    // error. We will then search the function parameters for a bound
                    // region at the right depth with the same index
                    (Some(rl::Region::LateBound(_, id, _)), ty::BrNamed(def_id, _)) => {
                        debug!("FindNestedTypeVisitor::visit_ty: LateBound",);
