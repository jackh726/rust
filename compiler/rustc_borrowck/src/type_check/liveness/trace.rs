...
use crate::polonius::{DeferredLocals, record_live_region_variance};
...
impl<'a, 'typeck, 'tcx> LivenessResults<'a, 'typeck, 'tcx> {
    ...
    fn dropck_boring_local(
        ...
        let local_span = typeck.body().local_decls[local].source_info.span;
    ...
    fn add_drop_live_facts_for(
        ...
        for &kind in &drop_data.dropck_result.kinds {
            make_all_regions_live(location_map, typeck, kind, live_at);
            polonius::legacy::emit_drop_facts(
                typeck.tcx(),
                dropped_local,
                &kind,
