...
impl<'a, 'tcx> LazyLiveness<'a, 'tcx> {
    ...
    fn materialize(
        ...
        //
        // Nothing in `tests/ui` observes this branch: disabling the widening altogether fails
        // ~200 tests, disabling only this fails none. A widened local's regions all outlive a
        // free region, so a loan reaching one is already live to the end of the body, which may
        // be why. That is missing coverage rather than evidence it is dead -- drop-liveness is
        // what keeps a loan alive across a local's last use -- so it stays.
        //
    ...
    /// Whether `region` is live at `point`, as far as has been materialized. Call `ensure` first.
