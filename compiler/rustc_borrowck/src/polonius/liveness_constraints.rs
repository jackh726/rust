...
/// Record the variance of each region contained within the given value, into `directions`.
///
/// Taking the directions rather than the whole context is what lets the traversal record them as
/// it materializes a deferred local's liveness, while it is holding the rest of the context.
