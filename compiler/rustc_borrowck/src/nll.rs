...
pub(crate) fn compute_regions<'tcx>(
    ...
    // Although this is done after `solve`, it's okay because liveness there is
    // *pessimistic*: any region outliving a universal region is also considered
    // live for the entire function. Any deferred regions *are* regions that fit
    // this category.
    if let Some(polonius_context) = polonius_context.as_mut() {
        ...
        regioncx.compute_loan_liveness(infcx, polonius_context, body, move_data, borrow_set);
    }
