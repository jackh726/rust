...
impl<'tcx> Binder<'tcx, ExistentialPredicate<'tcx>> {
    pub fn with_self_ty(&self, tcx: TyCtxt<'tcx>, self_ty: Ty<'tcx>) -> ty::Predicate<'tcx> {
        ...
        match self.skip_binder() {
            ExistentialPredicate::Trait(tr) => Binder(tr, std::marker::PhantomData)
                .with_self_ty(tcx, self_ty)
                .without_const()
                .to_predicate(tcx),
            ExistentialPredicate::Projection(p) => {
                Binder(p.with_self_ty(tcx, self_ty), std::marker::PhantomData).to_predicate(tcx)
            }
...
impl<'tcx> PolyTraitRef<'tcx> {
    ...
    pub fn to_poly_trait_predicate(&self) -> ty::PolyTraitPredicate<'tcx> {
        ...
        Binder(ty::TraitPredicate { trait_ref: self.skip_binder() }, std::marker::PhantomData)
    }
}
...
pub struct Binder<'tcx, T>(T, std::marker::PhantomData<&'tcx ()>);
...
impl<'tcx, T> Binder<'tcx, T> {
        ...
        T: TypeFoldable<'tcx>,
    {
        ...
        Binder(value, std::marker::PhantomData)
    }
    ...
    pub fn as_ref(&self) -> Binder<'tcx, &T> {
        Binder(&self.0, std::marker::PhantomData)
    }
    ...
    where
        F: FnOnce(T) -> U,
    {
        Binder(f(self.0), std::marker::PhantomData)
    }
    ...
    where
        F: FnOnce(T, U) -> R,
    {
        Binder(f(self.0, u.0), std::marker::PhantomData)
    }
    ...
    {
        let (u, v) = f(self.0);
        (Binder(u, std::marker::PhantomData), Binder(v, std::marker::PhantomData))
    }
}
...
impl<T> Binder<'tcx, Option<T>> {
    pub fn transpose(self) -> Option<Binder<'tcx, T>> {
        match self.0 {
            Some(v) => Some(Binder(v, std::marker::PhantomData)),
            None => None,
        }
    }
}
