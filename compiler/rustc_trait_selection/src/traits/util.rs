...
impl<'tcx> TraitAliasExpander<'tcx> {
    ...
    fn expand(&mut self, item: &TraitAliasExpansionInfo<'tcx>) -> bool {
        ...
        let pred = trait_ref.to_poly_trait_predicate().without_const().to_predicate(tcx);
        ...
        if item.path.iter().rev().skip(1).any(|&(tr, _)| {
            anonymize_predicate(tcx, tr.to_poly_trait_predicate().without_const().to_predicate(tcx))
                == anon_pred
        }) {
            return false;
        }
    ...
    fn next(&mut self) -> Option<DefId> {
        ...
        self.stack.extend(
            predicates
                .predicates
                .iter()
                .filter_map(|(pred, _)| pred.to_opt_poly_trait_ref(tcx))
...
pub fn closure_trait_ref_and_return_type(
    ...
    ty::Binder::bind((trait_ref, sig.skip_binder().output()), tcx)
}
...
pub fn generator_trait_ref_and_outputs(
    ...
    ty::Binder::bind((trait_ref, sig.skip_binder().yield_ty, sig.skip_binder().return_ty), tcx)
}
